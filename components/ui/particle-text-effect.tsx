...
class Particle {
  ...
  maxSpeed = 4.0 // Increased speed for responsiveness
  maxForce = 0.1
  particleSize = 6 // Slightly smaller particles for sharper text
  isKilled = false
  ...
  private generateRandomPos(x: number, y: number, mag: number): Vector2D {
    const randomX = Math.random() * 2000 // Increased range
...
const DEFAULT_WORDS = ["WELCOME", "TO MY", "PORTFOLIO", "CREATIVE", "DEVELOPER"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const pixelSteps = 5 // Lower is higher density
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    const newColor = {
      r: 255, // Keeping it white for high contrast on dark video
      g: 255,
      b: 255,
    }
  ...
  const animate = () => {
    ...
    ctx.clearRect(0, 0, canvas.width, canvas.height); // Clear instead of filling black to allow video background
    ...
    if (frameCountRef.current % 180 === 0) { // Faster switch
